...
# -*- coding: utf-8 -*-
"""
Created on Tue Jun  1 01:09:20 2021

@author: arpan

@Description: Train an Mini 3D Conv + GRU model on strokes. 
Model : Conv3DGRUClassifier
...
def train_model(model, dataloaders, criterion, encoder_optimizer, 
...
#                    break
            ...
            epoch_acc = running_corrects.double() / len(dataloaders[phase].dataset)  #(256*(bno+1)) #
...
#        inputs = inputs.permute(0, 4, 1, 2, 3).float()
        ...
        with torch.set_grad_enabled(phase == 'train'):
            ...
            enc_out, _ = model(inputs)
            gt_list.append(labels.tolist())
            pred_list.append((torch.max(enc_out, 1)[1]).tolist())
...
def main(DATASET, LABELS, CLASS_IDS, BATCH_SIZE, ANNOTATION_FILE, SEQ_SIZE=16, 
    ...
    model = conv_attn_model.Conv3DClassifier(HIDDEN_SIZE, 1, num_classes, bidirectional)
...
#    ###########################################################################
    ...
    model = train_model(model, data_loaders, criterion, optimizer, 
                        lr_scheduler, labs_keys, labs_values, num_epochs=N_EPOCHS)
        
    end = time.time()
    
    # save the best performing model
    attn_utils.save_model_checkpoint(base_name, model, N_EPOCHS, "SGD_conv3dgruMini_c5")
    # Load model checkpoints
    model = attn_utils.load_weights(base_name, model, N_EPOCHS, "SGD_conv3dgruMini_c5")
...
if __name__ == '__main__':
    ...
    extract_path = os.path.join(base_path, "conv3dgruMini_seq"+str(SEQ_SIZE))
    encoder =  main(DATASET, LABELS, CLASS_IDS, BATCH_SIZE, ANNOTATION_FILE, 
